...
export default function TerraNEXUSPage() {
  ...
  // Fetch álbumes (intenta pedir solo NEXUS al API y de todas formas filtra en cliente)
  useEffect(() => {
    let isMounted = true;
    setLoadingAlbums(true);
    fetch("/api/albums?project=NEXUS")
      .then((r) => r.json())
      .then((data) => {
        if (!isMounted) return;
        const arr: Album[] = Array.isArray(data) ? data : [];
        const onlyNEXUS = arr.filter((a) => (a.project ? a.project === "NEXUS" : true));
        setAlbums(onlyNEXUS);
      })
      .catch(() => setAlbums([]))
  ...
  // Fetch canciones (intenta pedir solo NEXUS y con álbum; si no, filtra en cliente)
  useEffect(() => {
    let isMounted = true;
    setLoadingSongs(true);
    fetch("/api/songs?project=NEXUS&withAlbum=1")
      .then((r) => r.json())
      .then((data) => {
        if (!isMounted) return;
        const arr: Song[] = Array.isArray(data) ? data : [];
        const withAlbum = arr.filter((s) => s.album && s.album.id);
        const onlyNEXUS = withAlbum.filter((s) => (s.album.project ? s.album.project === "NEXUS" : true));
        setSongs(onlyNEXUS);
      })
      .catch(() => setSongs([]))
  ...
  const filteredSongs = useMemo(() => {
    ...
    return songs.filter((s) => {
      const okNEXUS = s.album?.project ? s.album.project === "NEXUS" : true;

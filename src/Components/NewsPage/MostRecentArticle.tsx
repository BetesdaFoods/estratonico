...
function MostRecentArticle({ article }: { article: Article }) {
	return (
		<section className="flex flex-col gap-12 pt-16 sm:flex-row">
			...
			<div className="sm:w-1/2 flex flex-col justify-between">
				...
				<p className="py-3 sm:text-xl lg:text-2xl xl:text-3xl font-medium">
					<span className="line-clamp-5">{article.summary}</span>
				</p>
				<Button
					href={`/noticias/${article.id}`}
					className="uppercase py-4 px-14 sm:px-7 sm:py-6 lg:px-12 font-orbitron sm:text-2xl font-black self-start mt-6 text-nowrap"
				>
					Leer Artículo
				</Button>
			</div>

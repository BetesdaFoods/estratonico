...
export function NoticiasTitle() {
  const { idioma } = useLanguage();
  return (
    <h3 className="text-[25px] sm:text-4xl font-bold text-nowrap 2xl:text-[34px]">
...
export function SeeMoreButton({ link }: { link: string }) {
  ...
  return (
    <Link
			...
			className="flex items-center text-[#3BF3FF] font-bold xl:font-black text-nowrap lg:text-[25px] xl:text-lg 2xl:text-[20px] hover:underline hover:cursor-pointer select-none transform transition-transform duration-300 hover:xl:scale-105 origin-[0%_50%]"
		>
